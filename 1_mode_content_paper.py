...
SPHERICAL_MODES_PE = [(2, 2), (3, 2), (4, 2), (4, 4), (5, 4), (6, 4), (6, 6), (7, 6)]
...
def plot_mode_content_production(
    sim_id, mode_content_data_dict, t0_vals, spherical_modes, modes_to_plot=None
):
    ...
    full_modes_list = [
        list(map(tuple, inner)) for inner in mode_content_data_dict["modes"]
    ]
    ...
    # Create figure
    fig, ax = plt.subplots(
        figsize=(config.fig_width_2, config.fig_height_2 * (len(lm_groups) / 7)),
        dpi=300,
    )
    ...
    for group, modes in sorted_groups:
        ...
        y_labels.append(
            "QQNM"
            if group == ("QQNM",)
            else "CQNM" if group == ("CQNM",) else f"({group[0]},{group[1]})"
        )
        ...
        for mode in modes_set:
            if len(mode) == 4:
                ...
                if p == -1 and (l, m, n, -p) not in modes_set:
                    print(
                        f"Retrograde mode without prograde counterpart: {mode} at t0={t0}"
                    )
        ...
        for group, modes in sorted_groups:
            ...
            for n, mode in modes:
                ...
                if type == "qnm":
                    ...
                    hatch = (
                        "///////////"
                        if (mode in modes_set and (l, m, n, -1) in modes_set)
                        else None
                    )
    ...
    for label, pos in zip(ordered_labels, ordered_positions):
        if label == "(2,2,0)":
            ax.text(ax.get_xlim()[0] + 2.8 * dt, pos, label, va="bottom", ha="right")
            break
    ...
    plt.savefig(
        f"docs/front_page_figures/mode_content_{sim_id}.png", bbox_inches="tight"
    )
    plt.close()
...
def __main__():
    ...
    for sim_id in sim_ids:
        ...
        plot_mode_content_production(
            sim_id,
            mode_content_data_dict,
            t0_vals,
            spherical_modes,
            modes_to_plot=plotting_modes,
        )
