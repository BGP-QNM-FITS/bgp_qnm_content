...
def get_fits(
    sim_id, mode_content_data_dict, t0_vals, t0, full_modes_list, spherical_modes
):
...
def get_models_residuals(
    ...
    samples = np.random.multivariate_normal(
        mean_vector, covariance_matrix, int(num_draws)
    )
    residuals = np.zeros(
        (num_draws, len(spherical_modes), len(masked_data_array[0])),
        dtype=np.complex128,
    )
    models = np.zeros(
        (num_draws, len(spherical_modes), len(masked_data_array[0])),
...
def plot_residuals(
    sim_id, times, data, models, residuals, spherical_modes, plotting_modes, t0_choice
):
    """
    ...
    for mode in plotting_modes:
        fig, (ax_data, ax_resid) = plt.subplots(
            2,
            1,
            figsize=(config.fig_width, config.fig_height * 1.5),
            sharex=True,
        ...
        models_mode_real = np.abs(
            np.real(models[:, i, :])
        )  # shape: (n_samples, n_times)
        residuals_mode_real = np.abs(
            np.real(residuals[:, i, :])
        )  # shape: (n_samples, n_times)
        models_mode_imag = np.abs(
            np.imag(models[:, i, :])
        )  # shape: (n_samples, n_times)
        residuals_mode_imag = np.abs(
            np.imag(residuals[:, i, :])
        )  # shape: (n_samples, n_times)
        ...
        ax_data.plot(
            times, data_mode_real, color=special_color_1, alpha=0.5, label="Data"
        )
        ...
        ax_data.fill_between(
            times, p5_model, p90_model, color=special_color_1, alpha=0.3
        )
        ax_data.fill_between(
            times, p5_model_imag, p90_model_imag, color=special_color_2, alpha=0.3
        )

        ax_data.set_title(
            rf"$\ell = {mode[0]}, m = {mode[1]}$ at $t_0 = {t0_choice} \, [M]$"
        )
        ...
        ax_resid.fill_between(
            times, p5_resid_imag, p90_resid_imag, color="k", alpha=0.3
        )
...
def __main__():
    ...
    for sim_id in sim_ids:
        ...
        below_threshold_idx = next(
            (i for i, val in enumerate(ppc_vals) if val < PVAL_THRESHOLD), None
        )
