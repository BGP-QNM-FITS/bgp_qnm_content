...
for PLT_mode in target_sph_modes:
    ...
    for i in range(3):
        ...
        plt.savefig(
            f"{outdir}/PLT_trace_{i}_{PLT_modes[0][0]}{PLT_modes[0][1]}.png",
            bbox_inches="tight",
        )
        plt.close()
    ...
    plt.tight_layout()
    plt.savefig(
        f"{outdir}/PLT_corner_{PLT_modes[0][0]}{PLT_modes[0][1]}.png",
        bbox_inches="tight",
    )
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(
        f"{outdir}/PLT_histogram_{PLT_modes[0][0]}{PLT_modes[0][1]}.png",
        bbox_inches="tight",
    )
    plt.close()
    ...
    n_iid_samples = int(
        (samples.shape[0] - burn_in)
        * samples.shape[1]
        / (2 * np.max(autocorrelation_time))
    )
    print(
        f"Estimated number of independent and identically distributed (iid) samples: {n_iid_samples}"
    )
