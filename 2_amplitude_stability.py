...
l_to_color = {l: custom_cmap(i / (len(L_GROUPS) - 1)) for i, l in enumerate(L_GROUPS)}
...
def get_fits(sim_id, mode_content_data_dict, t0_vals, full_modes_list, spherical_modes):
    ...
    full_modes_list = [
        list(map(tuple, inner_list)) for inner_list in mode_content_data_dict["modes"]
    ]
...
def get_amplitude_stability_plot(
    fits, sim_id, mode_content_data_dict, plotting_modes, t0_vals, l_max
):
    ...
    for l in range(2, l_max + 1):
        for m in range(-l, l + 1):
            ...
            possible_modes_for_plot = (
                [
                    mode
                    for mode in unique_modes
                    if (len(mode) == 4 or len(mode) == 2)
                    and mode[0] == l
                    and mode[1] == m
                ]
                + [
                ...
                + [
                    ...
                    for mode in unique_modes
                    if len(mode) == 12
                    and (mode[0] + mode[4] + mode[8] == l)
                    and (mode[1] + mode[5] + mode[9] == m)
                ]
            )

            possible_modes_for_plot.sort(
                key=lambda mode: (mode[2] if len(mode) == 4 else float("inf"))
            )
            ...
            fig, ax = plt.subplots(
                figsize=(config.fig_width, config.fig_height), dpi=300
            )

            for mode in possible_modes_for_plot:
                label = (
                    nonlinear_modes[mode]
                    if mode in nonlinear_modes
                    else (
                        ""
                        if len(mode) == 2
                        else (
                            f"({mode[0]}, {mode[1]}, {mode[2]}, +)"
                            if np.sign(mode[3]) == 1
                ...
                for run in runs:
                    ...
                    for i, t0 in enumerate(temp_t0_vals):
                        ...
                        lowers[i] = np.percentile(
                            fits[tidx].fit["sample_amplitudes"][:, idx], 5
                        )
                        uppers[i] = np.percentile(
                            fits[tidx].fit["sample_amplitudes"][:, idx], 95
                        )
                    if len(temp_t0_vals) > 1:
                        ...
                        ax.plot(
                            [temp_t0_vals[0] - 1, temp_t0_vals[0] + 1],
                            [amps[0], amps[0]],
                            color=color,
                            alpha=alpha,
                            lw=lw,
            ...
            if threshold_idx is not None and threshold_idx + 1 < len(t0_vals):
                ...
                ax.axvspan(
                    0,
                    t0_vals[threshold_idx] - np.median(np.diff(t0_vals)) / 2,
                    color="grey",
                    alpha=0.2,
                    zorder=0,
                )
...
def __main__():
    ...
    for sim_id in sim_ids:
        ...
        full_modes_list = [
            list(map(tuple, inner_list))
            for inner_list in mode_content_data_dict["modes"]
        ]
        ...
        fits = get_fits(
            sim_id, mode_content_data_dict, t0_vals, full_modes_list, spherical_modes
        )

        plotting_modes = mode_rules_map[SPH_MODE_RULES[sim_id]]

        get_amplitude_stability_plot(
            fits, sim_id, mode_content_data_dict, plotting_modes, t0_vals, l_max
        )
