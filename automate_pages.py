...
descriptions = {
    "mode_content": (
        "Times QNMs are present is shown below. Solid lines indicate prograde modes, hatched lines indicate retrograde modes. "
        "Vertical grey shaded regions (where present) show poor fit times."
    ),
    "nonlinear_mode_mixing_alternatives": (
        "Prediction C of `Black-Hole Cartography <https://arxiv.org/abs/2410.13935>`_ is used. Results with predictions Bi, Bii, "
        "and D are also shown below."
    ),
    "amplitude_stability": (
        "Decay-corrected amplitudes are shown with the median as a bold line and 90% region shaded. "
        "Vertical grey shaded regions (where present) show poor fit times."
    ),
    "fits": (
...
for sim_folder in sorted(os.listdir(figures_dir)):
    ...
    if os.path.isdir(sim_path):
        ...
        if os.path.exists(json_path):
            ...
            spherical_modes = [
                tuple(mode) for mode in mode_content_data_dict["spherical_modes"]
            ]
            candidate_modes = [
                tuple(mode) for mode in mode_content_data_dict["candidate_modes"]
            ]
        ...
        mass_ratio, sim_name, spin1, spin2 = sim_metatdata.get(
            sim_id, ("N/A", "N/A", "N/A")
        )
        ...
        with open(rst_file, "w") as f:
            ...
            f.write(
                f"Tuples of length 2 / 4 / 8 / 12 are constant terms / QNMs / quadratic QNMs / cubic QNMs. \n\n"
            )
            f.write("::\n\n")
                ...
                if os.path.isdir(subfolder_path):
                    ...
                    if subfolder == "mode_content":
                        if sim_id == "0004":
                            ...
                            f.write(
                                "This simulation contains two instances of a retrograde mode present without a prograde mode, "
                            ...
                            f.write("Additional Notes\n----------------\n\n")
                            f.write(
                                "This simulation contains one instance of a retrograde mode present without a prograde mode, "
                                "which is not shown on the mode content plot. This is the (4, 4, 6, -) mode at timestep t0 = 2.0 [M]."
                                "\n\n"
                            )
                            ...
                            f.write(
                                "This simulation contains one instance of a retrograde mode present without a prograde mode, "
                                "which is not shown on the mode content plot. This is the (4, 4, 6, -) mode at timestep t0 = 4.0 [M]."
                                "\n\n"
                            )
                            ...
                            f.write("Additional Notes\n----------------\n\n")
                            f.write(
                                "**For this highly precessing system, we have found that QNM fits, and our algorithm, produce unreliable results. "
                            ...
                            f.write(
                                "This simulation contains multiple instances of a retrograde mode present without a prograde mode, "
                    ...
                    for file in sorted(os.listdir(subfolder_path)):
                        if file.endswith((".png", ".jpg", ".jpeg", ".pdf")):
                            rel_path = os.path.relpath(
                                os.path.join(subfolder_path, file), rst_dir
                            )
                            f.write(
                                f".. image:: {rel_path}\n   :width: 600px\n   :alt: {file}\n\n"
                            )
