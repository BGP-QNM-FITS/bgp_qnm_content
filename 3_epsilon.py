...
def plot_epsilon_main(
    sim_id, mode_content_data_dict, Mf_ref, chif_ref, t0_vals, fits, fits_full
):
    colors = LinearSegmentedColormap.from_list("custom_colormap2", config.colors2)(
        np.linspace(0, 1, 3)
    )
    ...
    threshold_idx = next(
        (i for i in reversed(range(len(p_values))) if p_values[i] > PVAL_THRESHOLD),
        None,
    )
    ...
    for i, t0 in enumerate(t0_vals):
        Mf_adaptive, chif_adaptive = (
            fits[i].fit["samples"][:, -1],
            fits[i].fit["samples"][:, -2],
        )
        Mf_full, chif_full = (
            fits_full.fits[i]["samples"][:, -1],
            fits_full.fits[i]["samples"][:, -2],
        )
        epsilon_adaptive = np.sqrt(
            (Mf_ref - Mf_adaptive) ** 2 / Mf_ref**2 + (chif_ref - chif_adaptive) ** 2
        )
        epsilon_full = np.sqrt(
            (Mf_ref - Mf_full) ** 2 / Mf_ref**2 + (chif_ref - chif_full) ** 2
        )
    ...
    ax_main.fill_between(
        t0_vals, p16_adaptive, p84_adaptive, color=colors[0], alpha=0.3
    )
...
def plot_epsilon_corners(sim_id, Mf_ref, chif_ref, t0_vals, fits, fits_full):
    ...
    for idx in range(0, len(t0_vals), 5):  # Loop through every fifth t0 value
        ...
        Mf_adaptive, chif_adaptive = (
            fits[idx].fit["samples"][:, -1],
            fits[idx].fit["samples"][:, -2],
        )
        Mf_full, chif_full = (
            fits_full.fits[idx]["samples"][:, -1],
            fits_full.fits[idx]["samples"][:, -2],
        )
        sns.kdeplot(
            x=Mf_adaptive,
            y=chif_adaptive,
